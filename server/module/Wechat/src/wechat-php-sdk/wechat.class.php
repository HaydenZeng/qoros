...
//		$this->log($postStr);
	...
	*/
	...
	public function news($newsData=array())
	{
        ...
        $this->log($this->getRevFrom());
        $this->log($this->getRevTo());
		$msg = array(
			'ToUserName' => $this->getRevFrom(),
			'FromUserName'=>$this->getRevTo(),
...
//            $expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
            //这里的时间匹配不对，原因未知，临时改成每天重新获取token
            $expire = 86400;
	...
	public function createMenu($data){
		...
		if ($result)
		{
			...
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
                $this->log('menu_error:'.$this->errCode.':'.$this->errMsg);
                $this->log('access_token:'.$this->access_token);
				return false;
			}
			return true;
		}
	...
	public function getOauthUserinfo($access_token,$openid){
		...
		if ($result)
		{
			...
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
                $this->log('error:'.$json['errcode'].'---'.$json['errmsg']);
				return false;
			}
			return $json;
		}
        $this->log('get user info error');
		return false;
	}
